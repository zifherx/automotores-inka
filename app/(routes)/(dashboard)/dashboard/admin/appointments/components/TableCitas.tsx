...
import { columnsCitas } from "@/table-columns/ColumnCitas";

export function TableCitas() {
  const { citas, isLoading, rangoFechas, setRangoFechas } = useCitas();
      ...
      <div className="flex flex-col gap-2 pb-4">
        ...
        <div className="flex flex-col md:flex-row items-start justify-start md:justify-end gap-1 w-full">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => toggleGrouping("tipoServicio")}
          >
            <ShoppingBasket className="w-5 h-5 mr-2" />
            {grouping.includes("tipoServicio") ? "Desagrupar" : "Agrupar"} por
            Tipo de Servicio
          </Button>
        </div>
      </div>
